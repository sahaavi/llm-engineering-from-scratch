...
class BPETokenizer:
    """Tiny byte-level BPE tokenizer for learning."""
...
def build_artifacts() -> dict[str, object]:
    ...
    write_json(ARTIFACTS / "trace.json", trace_data)
    write_json(ARTIFACTS / "metrics.json", metrics)
    write_json(Path(__file__).with_name("widget") / "data.json", trace_data)

    write_failure_gallery(ARTIFACTS / "failure_gallery.md", large_rows)
    draw_bar_chart(
        ARTIFACTS / "compression_ratio.png",
    ...
    draw_histogram(ARTIFACTS / "token_length_distribution.png", token_lengths)
    write_preview(ARTIFACTS / "preview.png")
    write_preview_gif(ARTIFACTS / "preview.gif")
